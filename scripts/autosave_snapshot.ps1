...
try {
    ...
    # --- L3 Signing-Readiness (2026-09-07) ------------------------------------
    # commit.gpgsign=true: git signiert ueber ssh-keygen (Repo-Konfig
    # gpg.ssh.program pinnt System OpenSSH). ERFUHRENE TATSACHE (2026-09-07):
    # Das Commit-Signieren entsperrt den Key ueber den ssh-agent ODER die
    # KEY-DATEI + Passphrase (via SSH_ASKPASS). Beide Wege gelten - der
    # Agent ist NICHT zwingend erforderlich (Repo pusht per HTTPS).
    # Die Passphrase liegt NICHT auf Disk, sondern im DPAPI-Store
    # (CurrentUser) und wird ueber scripts\signing_askpass.cmd
    # nicht-interaktiv geliefert.
    #
    # Sicherheitsnetz-Philosophie (siehe pre-commit-Header): der Autosave
    # wird NICHT blockiert. Ist der Key in diesem Kontext nicht
    # nicht-interaktiv entsperbar, laeuft der Snapshot mit --no-gpg-sign
    # (unsigned, laut geloggt) - Datensicherung geht vor Signatur.
    $noGpgSign  = $false
    $loadScript = Join-Path $RepoRoot 'scripts\load_signing_key.ps1'
    # Defensive: Askpass-Env garantieren (2026-09-07). Der autosave_watcher
    # ist ein langlaufender Prozess (Start beim Logon) und hat die setx-
    # Werte (SSH_ASKPASS / SSH_ASKPASS_REQUIRE) evtl. nicht geerbt. Ohne sie
    # kuennte git-eigenes Signieren in eine Passphrase-Wartehaenge laufen.
    # Der Askpass liest den DPAPI-Store - kein Klartext auf Disk.
    $env:SSH_ASKPASS         = Join-Path $RepoRoot 'scripts\signing_askpass.cmd'
    $env:SSH_ASKPASS_REQUIRE = 'force'
    # Best-effort: Key idempotent in den ssh-agent laden. FUR DAS SIGNIEREN
    # NICHT ERFORDERLICH (siehe oben), aber harmlos und nutz fuer SSH-Auth.
    if (Test-Path $loadScript) {
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $loadScript 2>&1 | Out-Null
    }
    # MASSGEBLICHER CHECK: Kann der Key in diesem Kontext nicht-interaktiv
    # entsperrt werden? Test mit EXAKT dem Tool, mit dem git signiert
    # (System ssh-keygen, gpg.ssh.program), per echtem Signier-Versuch auf
    # Dummy-Daten. stdin wird nach dem Payload-Write GESCHLOSSEN (kein
    # Prompt, kein Hang), Outputs werden konsumiert (kein Pipe-Voll-Stall).
    # Exit 0 = signierbar; sonst -> --no-gpg-sign (Sicherheitsnetz).
    $signReady  = $false
    $signingKey = (git -C $RepoRoot config user.signingkey 2>$null)
    if ($signingKey -and (Test-Path $signingKey)) {
        $psi = New-Object System.Diagnostics.ProcessStartInfo
        $psi.FileName          = 'C:\Windows\System32\OpenSSH\ssh-keygen.exe'
        $psi.Arguments         = "-Y sign -n git -f `"$signingKey`""
        ...
        try {
            ...
            $p.StandardInput.WriteLine('homebot-signing-readiness-check')
            $p.StandardInput.Close()
            $null = $p.StandardOutput.ReadToEnd()
            $null = $p.StandardError.ReadToEnd()
            $exited = $p.WaitForExit(15000)
            if ($exited) { $signReady = ($p.ExitCode -eq 0) } else { $p.Kill() }
        } catch {
            $signReady = $false
        }
    }
    if (-not $signReady) {
        $noGpgSign = $true
        Write-AutosaveLog 'WARNUNG: Signing-Key nicht nicht-interaktiv entsperbar (Askpass/DPAPI-Store) -> Snapshot OHNE GPG-Signatur'
    }
    ...
    try {
        if ($noGpgSign) {
            git -C $RepoRoot commit -q --no-gpg-sign -m "autosave $stamp ($changed Dateien)"
        } else {
