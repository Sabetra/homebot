...
| 4 | App-Startpfad intakt: .db_root + DB-Ordner zeigen auf homebot_dbs | Inspektion + Resolver-Smoke | ✅ 2026-09-04 (Smoke-Test: `%USERPROFILE%\.local\share\homebot_dbs`) |
...
| 2 | `.db_root`-Marker (live, außerhalb des Repos) | → `%USERPROFILE%\.local\share\homebot_dbs` | Resolver-Smoke ✅ |
...
| 2 | Resolver-Smoke (Produktiv-venv) | `get_project_root()` → `%USERPROFILE%\.local\share\homebot_dbs` (existiert) | 2026-09-04 |
