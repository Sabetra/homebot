...
> **Basis:** verifizierte Scans `%TEMP%\s2_scan.txt` (479 Z.) + `%TEMP%\s2_scan2.txt` (311 Z.) + frische `git grep` (2026-08-31)
...
> `user_context_builder/`). Vollständige Liste: `%TEMP%\s2_scan.txt` Abschnitt P3.
...
> **Scan-Artefakte (vollständige Datei:Zeile-Listen):** `%TEMP%\s2_scan.txt` (479 Z.), `%TEMP%\s2_scan2.txt` (311 Z.).
...
| 2026-09-01 | **C6** | `therapeutic_goals` → `care_goals` (18 Dateien, 96 Referenzen, DB-Migration) | §6.2 / Scope-B §14.1 | ✅ 946 passed; Migration idempotent; 0 Code-Treffer | `git revert` + DB-Backup `~\homebot_backups\db\auto\` |
...
| **DB-Backup** | `~\homebot_backups\db\auto\` | täglich (VACUUM INTO), 7 Stände — AGENTS.md §1b |
