...
> **Scope-Entscheidung (User, 2026-08-20):** Prüfen + Fehler beheben (kleinste Block-Ersetzung, Backup nach `%USERPROFILE%\<user>_backups\`, `last-verified` aktualisieren)
...
- Backup: `%USERPROFILE%\<user>_backups\` · git `autosave_watcher` (10-min-Snapshots)
...
|---|-------|----------|--------------|
| 1 | `AGENTS.md` (root) | L74 CJK `子系统` entfernt; L76: `07_KG_SOTA_ANALYSIS.md`→`02_SOTA_ROADMAP.md` §8–10 + `14_KG_...`; `last-verified: 2026-08-20` ergänzt | Editor-Diff ×3; Backup `%USERPROFILE%\<user>_backups\20260820_doku_audit\AGENTS.md` |

## Rollback

1. Vor Edit: `Copy-Item` → `%USERPROFILE%\<user>_backups\<name>.bak_20260820`
...
|---|---|---|
| AGENTS.md (root) | ✅ FIXED | L74 CJK entfernt; L76 `07_KG`→`02_SOTA_ROADMAP` §8–10 + `14_KG`; last-verified ergänzt; Backup `%USERPROFILE%\<user>_backups\20260820_doku_audit\AGENTS.md` |
...
| Backup (byte-exakt) | `%USERPROFILE%\<user>_backups\20260820_doku_audit\baseline_e6c0faac\` + `MANIFEST.json`; unabhängige Git-Blob-Hash-Verifikation **14/14 OK** |
