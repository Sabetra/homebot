...
| Backup-Ziel | `%USERPROFILE%\<user>_backups\` (nie ins Repo) |
...
- **Ergo:** Produktive DBs gehören unter `.db_root`
  (`%USERPROFILE%\.local\share\homebot_dbs`, via `utils/db_path_resolver`) — AGENTS.md.
...
|--------|----------|-----------|
| `.bak`-Dateien werden von Workflows erwartet (Rollback-Quelle) | mittel | Alle 29 Dateien nach `%USERPROFILE%\<user>_backups\repo_hygiene_20260828\<subpath>` kopiert **vor** `git rm`; Git-Historie enthält sie zusätzlich; Restore: `git checkout <sha> -- <pfad>`. |
...
- Die zu verschiebenden 2025er-DBs enthalten **potenzielle PII** (psychologische
  ...
  `%USERPROFILE%\<user>_backups\db\manual_20260828\` (nicht git-versioniert).
...
- **Q3:** Kanonische Backup-Strategie:
  ...
  2. Alle Verschiebungen → `%USERPROFILE%\<user>_backups\<kategorie>\<datum>\`
...
1. `%USERPROFILE%\<user>_backups\repo_hygiene_20260828\` anlegen.
...
3. 2025er-DB-Backups (6 Dateien, ~650 MB) →
   `%USERPROFILE%\<user>_backups\db\manual_20260828\` (inkl. aller `*.key`-Nachbarn),
   aus Repo entfernen.
...
> - DBs: 15 Dateien (~654 MB) → `%USERPROFILE%\<user>_backups\db\manual_20260828\`,
...
- **Phase 1:** `git revert <commit>` + Dateien aus
  `%USERPROFILE%\<user>_backups\repo_hygiene_20260828\` zurückkopieren. DBs: einfache
  `Copy-Item` zurück.
...
- **Phase 1 (Hygiene):** `refactored_gui/`, `intent_detector.py`,
  ...
  nach `%USERPROFILE%\<user>_backups\audit_20260828\` verschoben; 14 git-getrackte
  `.bak*`-Dateien per `git rm` entfernt (`.gitignore` deckt bereits ab); 4 alte
  psychologische Backup-DBs (2025) extern nach `%USERPROFILE%\<user>_backups\psych_2025\`.
...
`%USERPROFILE%\<user>_backups\audit_20260828\`).
...
Pre-Edit-Stände in `%USERPROFILE%\<user>_backups\kg_separation_20260828\`
...
Log (Beweis): `%USERPROFILE%\<user>_backups\wt_full_suite.log`.
