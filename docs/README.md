...
- **Befund:** Seit Einfuehrung von `.db_root` (2026-07-29) existierten produktive DBs
  doppelt: Resolver-Nutzer (Psycho, Chat, Finance) schrieben nach `homebot_dbs`,
...
- **Datenumzug** (App gestoppt, WAL-checkpointed, SHA256-verifiziert): lebende
  ...
  nach `homebot_dbs`; verwaiste Altstaende (inkl. Psycho-Altkopie **mit eigenem Key**)
  nach `~\homebot_backups\db\` archiviert — nichts geloescht.
- **Repo-Hygiene:** veralteter `psychological_support_kg_cache/` (18 MB Binaerdaten)
  ...
  automatisch dem Psycho-DB-Pfad in `homebot_dbs`.
