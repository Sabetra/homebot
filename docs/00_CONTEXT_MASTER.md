...
| 2026-09-12 | **Finance SOTA Phase 1 „Monarch-Core“**: Deterministische Cashflow-/Guthaben-Prognose im Finance-Tab. Schedule-first-Hybrid: wiederkehrende Zahlungen als deterministische Anker + variable Residuals (OLS-Trend × Monats-Saisonalität, Degradation < 12 Monate) + Residual-Bootstrap-KI (B=1000, fester Seed) + Guthaben-Kurve vom letzten `effective_balance_at` (nur IBAN + Einzelwährung). Neue Tools: `finance_upcoming_bills` (Fälligkeits-Kalender, Anchortag = Median letzter 5 Buchungstage), `finance_cash_flow_forecast` (1–24 Monate, KI-Band, Netto-Struktur), `finance_subscription_audit` (Monats-/Jahreskosten, Trend, Preisänderung, Abo-Heuristik). Registrierung: Schemas + Toolkit-Dispatch + `FINANCE_ANALYTICS`-Profil + Chat-Routing/Reflector/Planner (Fail-Fast-Validatoren) + Grammar-Compiler. UI: neuer Sub-Tab „📈 Prognosen“ (Plotly-KI-Band + Guthaben, Fälligkeits-Tabelle, Audit-Tabelle, Konto-Filter, Slider). i18n DE/EN/BG (`finance_ui.forecast.*`, 43 Keys). Tests: `tests/test_finance_monarch_core.py` (26 Tests: Saisonales Residual, Balance-Kette, CI-Reihenfolge, kein Future-Leak, Transfer-Ausschluss, Multi-Currency, Anker-/Fenster-Logik, Audit-Totale) + breitere Finance-Suite (13 Finance-Dateien + i18n + tool-profiles) **220/220 PASS** (2026-09-12). Abgelehnte Varianten (5 Kategorien × 1–7): ML (Overfit bei 12–36 Monatspunkten, Dependencies, Non-Determinismus), LLM-Forecast (Arithmetik, Reproduzierbarkeit). Doku: 03_FINANCE_MODULE.md §18, funktionen.md §AB; Workdoc: docs_archive/finance_sota_phase1_workdoc_20260912.md. |
| 2026-09-15 | **Finance SOTA Phase 2 „Sparziele“ (Goals/Sinking Funds)**: DB-backed Sparziele: Tabellen `goals` + `goal_contributions` (Cents-Integer, `UNIQUE(transaction_id)` = max. 1 Ziel pro Buchung, CASCADE; additive Auto-Migration). +10 Goal-Tools (5 read / 5 write: list/get/project, upsert/status/delete, assign/unassign/list_contributions, suggest_candidates) + Goals-Overlay in `finance_cash_flow_forecast` (`goals_draw`, `net_with_goals`, `balance_with_goals`); deklarierte Grenze: kein automatischer Stopp am Zielbetrag/-termin (03 §19). Registrierung: Schema-Katalog 10/10 + Toolkit-Dispatch + `FINANCE_ALL`/`FINANCE_ANALYTICS`; `finance_tab` read-only. UI: neuer Sub-Tab „🎯 Sparziele“ (`finance/tab.py::_render_goals_tab`): Anlage-Formular, Ziele-Tabelle mit Fortschritt, Status-Select, Projektion (erreicht > overdue > on/off-Track > Monate), Zuordnung freier Buchungen, Kandidaten-Vorschläge mit Pre-Fill. i18n DE/EN/BG (`finance_ui.goals.*`, 77 Keys + Tab-Key). Tool-Katalog: 37 → **47** Finance-Tools. Tests: `tests/test_finance_goals_schema.py` 43/43, `tests/test_finance_tab_regressions.py` 52/52 (inkl. 47 neuer reiner Helfer-Tests), Goals+i18n-Consistency 57/57, komplette Pre-Commit-Suite **1448/1448 PASS** (2026-09-15) + Release-Gate grün. Doku: 03_FINANCE_MODULE.md §19, funktionen.md (Sparziele-Tab-Sektion); Workdoc: docs_archive/WORKDOC_FINANCE_SOTA_PHASE2.md (archiviert). |
